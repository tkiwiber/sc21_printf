...
/*   Updated: 2020/07/14 15:52:19 by tkiwiber         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static void				ft_print_pct_get_strout(t_mask *mask)
{
	...
	i = -1;
	(mask->width > 1) ? (mask->plh_size = mask->width) :\
	(mask->plh_size = 1);
	(ft_strchr(mask->flag, '-')) ? (beg = 0) :\
	(beg = mask->plh_size - 1);
	uc = '%';
	...
	while (++i < mask->plh_size)
		(i == beg) ? (ft_putchar_fd(uc, 1)) :\
		(ft_putchar_fd(mask->plh_w_ch, 1));
}
